...
#define STCDC 0x60 //A/D converter and pol l Status
...
uint8_t adcv_degerler[6]={0};
uint8_t volt_deger[18] = {0};
...
void yaz()
{
	...
	HAL_SPI_Transmit(&hspi2, data, 8, 1000);
...
void oku()
{
	...
	HAL_SPI_Transmit(&hspi2, data2, 2, 1000);
	HAL_SPI_Receive(&hspi2, adcv_degerler, 6, 1000);
...
void v_oku()
{
	...
	data3[0] = STCVAD;
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, RESET);
	HAL_SPI_Transmit(&hspi2, data3, 1, 1000);
	HAL_Delay(20);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, SET);
	uint8_t data4[2];
	data4[0] = address;
	data4[1] = RDCFG;
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, RESET);
	HAL_SPI_Transmit(&hspi2, data4, 2, 1000);
	HAL_SPI_Receive(&hspi2, volt_deger, 18, 1000);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, SET);
	if(HAL_Status == HAL_OK)
	{
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, SET);
